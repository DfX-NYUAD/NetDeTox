# llm_exo.py
# LLM-driven gate-combo decisions (based on historical performance only; no KL, no combo-RL)
# Fixed allow/block lists are loaded at runtime (file/env vars); each round the LLM is explicitly told
# the Allowed/Blocked sets and is forced to choose only from Allowed.
...
class ExperimentLogger:
    """
    Line-based JSON log (JSONL), one record per line, convenient for later analysis and
    safe for concurrent writes. Defaults to work_dir/run_log.jsonl.
    """
...
# ===== Gate-combo: RL disabled, use LLM decisions instead (no KL) =====
...
EPISODE_STEPS = 10
SEC_DROP_MIN_REL = float(os.getenv("SEC_DROP_MIN_REL", "5e-4"))  # threshold for "not dropping fast enough"
...
STICKY_EMA_DS = float(os.getenv("STICKY_EMA_DS", "5e-4"))        # stickiness threshold for a combo with strong history
...
GATE_COMBOS_AREA: Dict[str, str] = {
    ...
    # Add lightweight combinations if more coverage is required
...
def llm_call(prompt: str, system_prompt: str, temperature: float = 0.8) -> str:
    """
    Without OPENAI_API_KEY, fall back locally: pick from Candidates (already filtered to Allowed)
    using a heuristic ranking.
    """
...
#             # Look for content of type "message"
...
# ================== RL: simple multi-arm policy (per bucket) ==================
...
def llm_choose_from_pool(netlist: str, pool: List[str], try_per_round: int,
    ...
    if not valid:
        return []
    # Prime the feature cache for later combo/k decisions (callers may re-invoke extract; not returned here)
    _ = [extract_node_features(netlist, inst) for inst in valid[:1]]  # trigger caching
...
# ================== combo history persistence (no KL, history-only) ==================
...
def llm_choose_combo_by_history(
    ...
    *,
    ...
    # ============== Allow/block lists & candidate assembly (same as original logic) ==============
    ...
    for name in allowed_names:
        ...
        cand.append({
            ...
            "ema_ds": float(hist.get("ema_ds",0.0)),  # +: security drop (higher is better)
            "ema_da": float(hist.get("ema_da",0.0)),  # +: area drop (higher is better)
    ...
    # Append the picked nodes and pool info to the Context of system_prompt
    ctx_extra = {
    ...
    # ===== Fallback/correction (same as before, with strategy merged in) =====
    if choice is None:
    ...
    # A) When security is below threshold, prioritize "slight area decrease"
...
def llm_choose_combo_area_priority(
    ...
    *,
    ...
    """
    LLM area-priority: prefer combos that reduce area, subject to "not worsening security".
    - Hard preference: ema_da > 0
    - Security floor: ema_ds >= -SEC_TOL
    - If security recently shows signs of worsening or a switch is needed, avoid repeating the last combo
    - Automatically prefers GATE_COMBOS_AREA (if it exists), otherwise falls back to the original GATE_COMBOS
    """
    import json

    SEC_TOL  = float(os.getenv("SEC_NO_WORSE_TOL", "5e-4"))   # tiny negative security fluctuation allowed
    AREA_MIN = float(os.getenv("AREA_IMPROVE_MIN", "5e-3"))   # prioritize significant area drops
    SMALL_AREA_DROP = float(os.getenv("SMALL_AREA_DROP", "0.02"))

    # === Key change: choose the "area-friendly" combo table, fall back if absent ===
    ...
    for name in allowed_names:
        ...
        cand.append({
            ...
            "trials": int(hist.get("n",0)),
            "ema_ds": float(hist.get("ema_ds",0.0)),  # +: security drop
            "ema_da": float(hist.get("ema_da",0.0)),  # +: area drop
            "last_ds": float(hist.get("last_ds",0.0)),
            "last_da": float(hist.get("last_da",0.0)),
            "succ": int(hist.get("succ",0)),
            "streak": int(hist.get("streak",0)),
    ...
    # ---------- Local fallback (area-priority + security floor) ----------
    if choice is None:
...
def llm_choose_k_area_priority(
    *,
    ...
    """
    LLM area-priority: choose k to drive area down without worsening security.
    Empirical rules (also written into the prompt):
    - If area has not clearly dropped recently -> increase k moderately
    - If security shows signs of worsening -> do not just keep raising it; prefer small-to-mid k for stable fine-tuning
    - If area is dropping and security is stable/improving -> prefer smaller k
    """
    ...
    if ema_ds < -SEC_TOL:
        ...
        k += 1  # area not clearly dropping -> increase moderately
    else:
        k -= 1  # area dropping and security stable -> smaller k
...
#     pool_digest: Optional[Dict[str, Any]] = None,
#     llm_temperature: float = 0.6,
#     fallback_temp: float = 0.6,       # fallback softmax temperature, to avoid always taking the top item
#     force_explore_prob: float = 0.10  # small probability of exploring an item other than argmax
# ) -> str:
#     """
#     Pick a combo from the fixed allow/block lists + historical performance, but also give the LLM
#     the context of the "confirmed picks", deciding with a security-first, area-aware strategy.
#     If the LLM fails, fall back to soft sampling based on node fit + history.
#     """
...
#     allowed_names = [n for n in sorted(list(allowed_names)) if n not in block_set]
#     blocked_names = sorted(list(block_set))
#     if not allowed_names:  # when allow is empty, fall back to "all minus blocklist"
#         allowed_names = [n for n in GATE_COMBOS.keys() if n not in block_set] or GATE_COMBO_NAMES[:]
...
#             "bad": _bad_combo(name)
#         })
#     # For display/fallback ordering only (the LLM does not depend on this ordering)
#     cand.sort(key=lambda x: (not x["bad"], x["ema_ds"], x["ema_da"], x["succ"], x["best_ds"], -x["trials"]), reverse=True)
...
#     # Recent changes for the LLM to reference
#     def _tail(xs, n=8):
#         xs = list(xs or [])
...
#         # First take one softmax sampling result
...
#     # ---------- 7) When security is below threshold, fine-tune: prefer slight area decrease ----------
#     if security <= SEC_FOCUS_THRESHOLD:
#         cur = next((z for z in cand if z["combo"] == choice), None)
...
def llm_choose_k(
    *,
    ...
    No manual mapping or soft bandit; just pass the combo info as context and let the LLM decide.
    """
    ds_hist = list(recent_ds_rel or [])
    da_hist = list(recent_da_rel or [])
    ema_ds = _ema_list(ds_hist) if ds_hist else 0.0
    ...
    # This combo's historical performance (if any)
    ...
    # ======== Local fallback (does not depend on a combo mapping) =======
...
def _parse_security_from_text_gnnre(text: str) -> float:
    ...
    Handles multiple namings: F1_Micro / F1 Micro / micro-F1 / micro_f1 / micro f1.
    Returns a float, or 0.0 on parse failure.
    """
    if not text:
        return 0.0

    # 1) Prefer looking in the [RESULT] line (if the final output uses a unified result line)
    m = re.search(
    ...
    # 2) Global match for F1_Micro-style patterns
    patterns = [
    ...
    # 4) Fallback: scan the last 100 lines and take the trailing number on lines containing "micro"
    try:
...
def _parse_security_from_text_trojan(text: str) -> float:
    ...
    Returns a float, or raises on parse failure.
    """
    if not text:
        return 0.0
...
def run(initial_netlist: Any, max_iters: int=MAX_ITERS, *,
    ...
    # Track the most recent "real evaluation" security (used to hold it on non-refresh rounds)
    ...
    for t in range(max_iters):
        ...
        # ---------- Early stop (only valid for the corresponding backend) ----------
        # Note: for gnnre, only check early stop on "refresh rounds" to avoid using the "held" security.
        ...
        # ---------- Select gate combo (step two; pass in picks / pool_digest) ----------
        ...
        # ---------- Select k (step three; pass in picks / pool_digest / combo) ----------
...
def run_area(initial_netlist: Any, max_iters: int=20, *,
    ...
    Goal: reduce area without worsening security.
    """
    ...
    for t in range(max_iters):
        ...
        chosen_k = llm_choose_k_area_priority(
            ...
            combo_name=chosen_combo_name,
            use_gates=chosen_use_gates,
            prefer_small_k=True,   # bias small to reduce complexity
            k_min=1, k_max=3,
            last_k=last_k_used,
            picked_nodes=picks, pool_digest=pool_digest
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--work_dir", type=str, default=os.environ.get("WORK_DIR","tmp"),
                        help="Working directory for subcircuit_opt (e.g. tmp) - the evaluator internally uses ../{work_dir}")
    ...
    # # === Stage 2: reduce area (while ensuring security does not worsen) ===
