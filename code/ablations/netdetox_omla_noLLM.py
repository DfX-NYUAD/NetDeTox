# llm_exo.py
# LLM-driven gate combo selection (based only on historical performance; no KL, no combo-RL)
# Fixed allow/block lists are loaded at runtime (file/environment variables); each round the LLM is
# explicitly told Allowed/Blocked and is forced to choose only from Allowed.
...
class ExperimentLogger:
    """
    Line-oriented JSON logging (JSONL), one record per line, convenient for later stats and
...
# ===== Gate-combo: disable RL, use LLM decisions instead (no KL) =====
...
EPISODE_STEPS = 10
SEC_DROP_MIN_REL = float(os.getenv("SEC_DROP_MIN_REL", "5e-4"))  # threshold for deciding "not dropping fast"
...
STICKY_EMA_DS = float(os.getenv("STICKY_EMA_DS", "5e-4"))        # stickiness threshold for combos with strong history
...
# ===== Early Stop (applies only to OMLA) =====
...
# ================== Gate sets (includes BUF to avoid occasional missing-gate errors) ==================
...
GATE_COMBOS_AREA: Dict[str, str] = {
    # Basic cells (smallest area)
    "C01_INV_BUF"    : "INV,BUF",
    "C02_NAND_ONLY"  : "INV,NAND,BUF",
    "C03_NOR_ONLY"   : "INV,NOR,BUF",

    # NAND/NOR + simple AND/OR logic (area still low)
    "C04_INV_NAND_AND": "INV,NAND,AND,BUF",
    ...
    # If wider coverage is required, a lightweight combo can be added
...
def llm_call(prompt: str, system_prompt: str, temperature: float = 0.8) -> str:
    """
    When OPENAI_API_KEY is not set, fall back locally: pick from Candidates (already filtered to Allowed)
    by heuristic ranking.
    """
...
#     print(f"Reasoning tokens: {reasoning_tokens}, Total tokens: {total_tokens}")
...
#                         print(f"Found message text: {text_content}")
#                         return text_content
            
#             # Original text-type check
#             elif isinstance(item, dict) and item.get("type") == "text" and "content" in item:
#                 content = item["content"]
#                 print(f"Found text content: {content}")
#                 return content
    
#     # Fallback: return the reasoning result or empty JSON
#     if reasoning_tokens > 0 and "reasoning" in data:
#         reasoning_summary = data["reasoning"].get("summary", "")
...
# ================== RL: simple two-arm policy (per bucket) ==================
...
# ================== RL over hop size k (softmax gradient bandit) ==================
class RLKPolicy:
...
# ================== Candidate pool & LLM refinement (nodes) ==================
...
def llm_choose_from_pool(netlist: str, pool: List[str], try_per_round: int,
    ...
    if not valid:
        return []
    # Prime the feature cache for later combo/k decisions (callers may re-invoke extract; not required to return)
...
# ================== Fixed/history file paths & loading (runtime) ==================
...
# ================== Combo historical-performance persistence (no KL; history only) ==================
_COMBO_PERF: Dict[str, Dict[str, float]] = {}
_COMBO_PERF_BETA = float(os.getenv("COMBO_EMA_BETA","0.2"))  # EMA smoothing
SUCCESS_TOL = float(os.getenv("SUCCESS_TOL","1e-4"))         # ds_rel threshold for counting as "effective"
...
# ================== LLM-driven gate combo (history only + fixed allow/block lists) ==================
def llm_choose_combo_by_history(
    ...
    *,
    ...
    picked_nodes: Optional[List[str]] = None,          # <<< new
    pool_digest: Optional[Dict[str, Any]] = None       # <<< new (e.g. {"size":N,"fingerprint": "..."})
) -> str:
    # Reuse / fallback thresholds
    SEC_FOCUS_THRESHOLD = float(os.getenv("SEC_FOCUS_THRESHOLD", os.getenv("EARLY_STOP_FOR_OMLA", "0.5")))
    SMALL_AREA_DROP = float(os.getenv("SMALL_AREA_DROP", "0.02"))  # upper bound for a "slight area decrease", ~2%
    AREA_TOL_UP     = float(os.getenv("AREA_TOL_UP",   "0.01"))    # allowed small area increase when balancing, <=1%
    ...
    for name in allowed_names:
        ...
        cand.append({
            ...
            "ema_ds": float(hist.get("ema_ds",0.0)),  # +: security decreases (higher is better)
            "ema_da": float(hist.get("ema_da",0.0)),  # +: area decreases (higher is better)
    ...
    # ============== Feed "history" into the system prompt: recent_* and their EMAs ==============
    ...
    # Whether to trigger the balancing strategy for "last round area dropped but security rose a lot"
    ...
    # Append picked-node and pool info to the Context of system_prompt
    ctx_extra = {
    ...
    # ===== Fallback/correction (same as before, with strategy folded in) =====
    if choice is None:
    ...
    if security <= SEC_FOCUS_THRESHOLD:
        ...
        if good:
            good.sort(key=lambda x: (
                ...
                x["ema_da"],         # smaller means more "slight"
...
def llm_choose_combo_area_priority(
    ...
    *,
    ...
    """
    LLM area-first: subject to "not worsening security", prefer combos that reduce area.
    - Hard preference: ema_da > 0
    - Security floor: ema_ds >= -SEC_TOL
    - If security shows recent signs of worsening or a switch is needed, avoid repeating last round's combo
    - Automatically prefer GATE_COMBOS_AREA (if present); otherwise fall back to the original GATE_COMBOS
    """
    import json

    SEC_TOL  = float(os.getenv("SEC_NO_WORSE_TOL", "5e-4"))   # allow a tiny negative fluctuation in security
    AREA_MIN = float(os.getenv("AREA_IMPROVE_MIN", "5e-3"))   # prioritize a significant area decrease
    SMALL_AREA_DROP = float(os.getenv("SMALL_AREA_DROP", "0.02"))

    # === Key change: pick the "area-friendly" combo table; fall back if absent ===
    ...
    for name in allowed_names:
        ...
        cand.append({
            ...
            "trials": int(hist.get("n",0)),
            "ema_ds": float(hist.get("ema_ds",0.0)),  # +: security decreases
            "ema_da": float(hist.get("ema_da",0.0)),  # +: area decreases
            "last_ds": float(hist.get("last_ds",0.0)),
            "last_da": float(hist.get("last_da",0.0)),
            "succ": int(hist.get("succ",0)),
            "streak": int(hist.get("streak",0)),
...
def llm_choose_k_area_priority(
    *,
    ...
    """
    LLM area-first: choose k so that area decreases without worsening security.
    Rules of thumb (also written into the prompt):
    - If area hasn't dropped noticeably recently -> increase k moderately
    - If security shows signs of worsening -> don't just keep increasing; prefer small-to-mid k for stable fine-tuning
    - If area is dropping and security is unchanged/improving -> prefer a smaller k
    """
    ...
    ema_da = _ema_list(da_hist) if da_hist else 0.0
    ...
    if ema_ds < -SEC_TOL:
        k += 0  # avoid large k; keep small-to-mid
    elif ema_da <= AREA_TOL:
        k += 1  # area hasn't dropped noticeably -> increase moderately
    else:
...
#     pool_digest: Optional[Dict[str, Any]] = None,
#     llm_temperature: float = 0.6,
#     fallback_temp: float = 0.6,       # fallback softmax temperature, to avoid always taking the first
#     force_explore_prob: float = 0.10  # small probability of exploring an item other than argmax
# ) -> str:
#     """
#     Pick a combo from the fixed allow/block lists + historical performance, but also give the LLM the
#     context of the "confirmed picks", deciding with a security-first, area-aware strategy. If the LLM
#     fails, fall back to soft sampling based on node fit + history.
#     """

#     # ---------- 0) Constants ----------
#     SEC_FOCUS_THRESHOLD = float(os.getenv("SEC_FOCUS_THRESHOLD", os.getenv("EARLY_STOP_FOR_OMLA", "0.5")))
#     SMALL_AREA_DROP = float(os.getenv("SMALL_AREA_DROP", "0.02"))
#     AREA_TOL_UP     = float(os.getenv("AREA_TOL_UP",   "0.01"))
#     SEC_BLOWUP_ABS  = float(os.getenv("SEC_BLOWUP_ABS","0.05"))
...
#     allowed_names = [n for n in sorted(list(allowed_names)) if n not in block_set]
#     blocked_names = sorted(list(block_set))
#     if not allowed_names:  # when allow is empty, fall back to "all minus block list"
#         allowed_names = [n for n in GATE_COMBOS.keys() if n not in block_set] or GATE_COMBO_NAMES[:]

#     # ---------- 2) Candidates and history ----------
#     cand: List[Dict[str, Any]] = []
#     for name in allowed_names:
#         hist = _COMBO_PERF.get(name, {})
#         cand.append({
...
#             "bad": _bad_combo(name)
#         })
#     # For display / fallback ordering only (the LLM does not rely on this ordering)
#     cand.sort(key=lambda x: (not x["bad"], x["ema_ds"], x["ema_da"], x["succ"], x["best_ds"], -x["trials"]), reverse=True)

#     # ---------- 3) Node context based on picks ----------
#     def _safe_feat(nm: str):
#         try:
#             return extract_node_features(netlist_path=pool_digest.get("__netlist__", "") or "", inst_name=nm)
#         except Exception:
#             return {"gate":"UNK","fanin":0,"fanout":0,"level":0}

#     feats = [_safe_feat(n) for n in (picked_nodes or [])]
#     if not feats:
#         feats = [{"gate":"UNK","fanin":0,"fanout":0,"level":0}]
#     import collections, math, random
#     lv = [float(f.get("level",0)) for f in feats]
#     fo = [float(f.get("fanout",0)) for f in feats]
#     fi = [float(f.get("fanin",0))  for f in feats]
#     def _stat(xs):
#         xs = list(xs or [0.0]); xs_sorted = sorted(xs)
#         return {
#             "mean": sum(xs)/max(1,len(xs)),
#             "max":  xs_sorted[-1] if xs_sorted else 0.0,
#             "p75":  xs_sorted[int(0.75*(len(xs_sorted)-1))] if xs_sorted else 0.0
#         }
#     gate_hist = collections.Counter([str(f.get("gate","UNK")).upper() for f in feats])
#     base_cnt = sum(1 for n in (picked_nodes or []) if IS_BASE(n))
#     opt_cnt  = sum(1 for n in (picked_nodes or []) if IS_OPT(n))
#     node_ctx = {
#         "n_nodes": len(picked_nodes or []),
#         "level": _stat(lv),
#         "fanout": _stat(fo),
#         "fanin": _stat(fi),
#         "is_base_ratio": base_cnt / max(1, base_cnt+opt_cnt),
#         "is_opt_ratio":  opt_cnt  / max(1, base_cnt+opt_cnt),
#         "gate_hist_top": gate_hist.most_common(8),
#         "pool_digest": pool_digest or {}
#     }

#     # ---------- 4) LLM context and rules ----------
#     prefer_rule = (
#         "Security dropped too slowly last round. DO NOT pick '{last}'. "
#         "Choose ONLY from Allowed; never from Blocked. Prefer higher ema_ds."
#     ) if (need_switch and last_combo) else (
#         "Choose ONLY from Allowed; never from Blocked. Prefer higher ema_ds (more security drop), "
#         f"and slight area drop when possible (ema_da>0, ideally <= {SMALL_AREA_DROP:.3f})."
#     )

#     # Recent changes for the LLM's reference
#     def _tail(xs, n=8): 
#         xs = list(xs or [])
#         return xs[-n:] if xs else []

#     system_prompt = (
#         "You are a gate-combo selection agent.\n"
#         "Output ONLY valid compact JSON: {\"combo\": \"NAME\"}. No commentary.\n"
#         "Rules:\n"
#         f"- {prefer_rule.replace('{last}', last_combo or '')}\n"
#         "- Do not invent names; pick exactly one from Allowed.\n"
#         "- Avoid items with bad=true when a good option exists.\n"
#         "- Consider the picked nodes' structure (levels/fanout/fanin/gate distribution) when matching combos.\n"
#         "- When security is already low, prefer slight area decrease.\n"
#         "Context:\n"
#         + json.dumps({
#             "current_security": float(security),
#             "current_area": float(area),
#             "recent_changes_old_to_new": [
#                 {"ds_rel": float(ds), "da_rel": float(da)}
#                 for ds, da in zip(_tail(recent_ds_rel,8), _tail(recent_da_rel,8))
#             ],
#             "last_combo": last_combo,
#             "need_switch": bool(need_switch),
#             "nodes_context": node_ctx
#         }, ensure_ascii=False)
#         + "\nAllowed (with stats):\n"
#         + json.dumps(
#             [{"combo": c["combo"], "use_gates": c["use_gates"], "ema_ds": c["ema_ds"], "ema_da": c["ema_da"],
...
#     # ---------- 5) Let the LLM actually make the choice ----------
#     choice = None
#     try:
#         data = json.loads(llm_call(prompt, system_prompt=system_prompt, temperature=float(llm_temperature)))
#         c = str(data.get("combo","")).strip()
...
#     # ---------- 6) If the LLM fails -> fall back (node fit + history, softmax sampling) ----------
#     def _gate_presence_score(use_gates: str) -> float:
#         if not gate_hist: return 0.0
#         use = set([g.strip().upper() for g in (use_gates or "").split(",") if g.strip()])
#         tot = sum(gate_hist.values()) or 1
#         score = 0.0
#         for g, cnt in gate_hist.items():
#             if g in use:
#                 score += cnt / tot
#         return score

#     def _score_combo(c: Dict[str,Any]) -> float:
#         ema_ds = float(c.get("ema_ds",0.0))
#         ema_da = float(c.get("ema_da",0.0))
#         bad = bool(c.get("bad",False))
#         fit = _gate_presence_score(c.get("use_gates",""))
#         base = 2.5*ema_ds + 1.2*fit + 1.0*ema_da
#         if bad: base -= 0.8
#         if last_combo and need_switch and c["combo"] == last_combo:
#             base -= 0.5
#         if security <= SEC_FOCUS_THRESHOLD and ema_da > 0.0:
#             base += (0.3 if 0.0 < ema_da <= SMALL_AREA_DROP else 0.05)
#         return base

#     if (choice is None) or (choice in block_set):
#         if not cand:
#             return "C14_INV_AND_OR_LOGIC"
#         import math, random
#         scs = {c["combo"]: _score_combo(c) for c in cand}
#         mx = max(scs.values()) if scs else 0.0
#         exps = {k: math.exp((v - mx)/max(1e-6, float(fallback_temp))) for k,v in scs.items()}
#         Z = sum(exps.values()) or 1.0
#         ranked = sorted(((k, exps[k]/Z) for k in exps), key=lambda x: -x[1])
#         # First take a softmax sampling result
#         r = random.random(); acc = 0.0; sampled = ranked[0][0]
#         for k,p in ranked:
#             acc += p
#             if r <= acc:
#                 sampled = k; break
#         # With small probability, force exploration of the second-best
#         if random.random() < float(force_explore_prob) and len(ranked) >= 2:
#             sampled = ranked[1][0]
#         choice = sampled

#     # ---------- 7) Fine-tune when security is below threshold: prefer a slight area decrease ----------
#     if security <= SEC_FOCUS_THRESHOLD:
#         cur = next((z for z in cand if z["combo"] == choice), None)
#         good = [x for x in cand if (x["ema_da"] > 0.0) and (x["combo"] not in block_set) and (not x["bad"])]
#         if good:
#             good.sort(key=lambda x: (
#                 0 if (0.0 < x["ema_da"] <= SMALL_AREA_DROP) else 1,
#                 x["ema_da"], -x["ema_ds"], -x["ema_da"]
#             ))
#             need_replace = (cur is None) or (cur["ema_da"] <= 0.0) or (cur["bad"]) or (choice in block_set)
#             if not need_replace and (cur["ema_da"] > SMALL_AREA_DROP):
#                 need_replace = True
...
#     # ---------- 8) Last round area dropped but security rose a lot -> balance ----------
#     if (last_da_rel is not None and last_ds_rel is not None
#         and last_da_rel > 0.0 and last_ds_rel < -SEC_BLOWUP_ABS):
#         cur = next((z for z in cand if z["combo"] == choice), None)
#         bal = [x for x in cand if (x["ema_da"] >= -AREA_TOL_UP) and (x["combo"] not in block_set) and (not x["bad"])]
#         if bal:
#             bal.sort(key=lambda x: (
#                 0 if (x["ema_da"] > 0.0 and x["ema_da"] <= SMALL_AREA_DROP) else
#                 (1 if (x["ema_da"] >= -AREA_TOL_UP and x["ema_da"] <= 0.0) else 2),
#                 -x["ema_ds"], abs(x["ema_da"]), -x["ema_da"]
#             ))
#             need_replace = (
#                 cur is None or cur["bad"] or (choice in block_set) or
#                 (cur["ema_da"] < -AREA_TOL_UP)
...
def llm_choose_k(
    *,
    ...
    last_k: Optional[int] = None,
    picked_nodes: Optional[List[str]] = None,      # <<< new
    pool_digest: Optional[Dict[str, Any]] = None   # <<< new
) -> int:
    """
    The LLM chooses an integer k in [k_min, k_max] (default 1..10).
    No manual mapping or soft bandwidth; just pass the combo info as context and let the LLM decide.
    """
    ds_hist = list(recent_ds_rel or [])
    da_hist = list(recent_da_rel or [])
    ema_ds = _ema_list(ds_hist) if ds_hist else 0.0
    ...
    # ======== Local fallback (independent of combo mapping) =======
    # Default 5; security not dropping -> +2; dropping slowly -> +1; area dropping and security dropping -> -1
    # k = 5
    # if ema_ds <= 0.0:
    #     k += 2
...
# ================== Optimizer invocation (serial stacking) ==================
...
#         res = _call_optimizer_once(curr, inst, iter_index=iter_index,
#                                    override_k=override_k, override_use_gates=override_use_gates, eval_backend=eval_backend)
#         notes.append(f"[{inst}] {res.note[-160:]}")
#         if not res.success:
#             return OptimizationResult(curr, False, "\n".join(notes))
    ...
    for inst in inst_names:
        try:
            ...
            if res.success:
                ...
                # Error: record it, keep curr, do not interrupt
                notes.append(f"[{inst}] failed, keep current netlist ({curr})")
        except Exception as e:
            # Exception: record it, keep curr, do not interrupt
            notes.append(f"[{inst}] EXCEPTION {e}, keep current netlist ({curr})")

    return OptimizationResult(curr, True, "\n".join(notes))


# ================== Evaluation backends (OMLA & GNN4IP) ==================
...
def _parse_security_from_text_gnnre(text: str) -> float:
    ...
    Handles several naming variants: F1_Micro / F1 Micro / micro-F1 / micro_f1 / micro f1.
    Returns a float, or 0.0 if parsing fails.
    """
    if not text:
        return 0.0

    # 1) First look in the [RESULT] line (if the final output uses a unified result line)
    m = re.search(
    ...
    # 2) Global match for the F1_Micro style
    patterns = [
    ...
    # 3) Restrict to near the "Full test stats" block (if present), searching the following few lines
    ...
    # 4) Fallback: scan the last 100 lines, taking the end-of-line number on lines containing "micro"
    try:
...
def _parse_security_from_text_trojan(text: str) -> float:
    ...
    if m:
        try:
            ...
            pass
    # Fallback: find the number on the last line
...
# Unified tunable reward weights (used only by node-bucket RL and the k-bandit)
    ...
    payload = {
        ...
        "selection": {
            "combo": {
                ...
                "history_snapshot": combo_hist   # n, ema_ds, ema_da, last_ds, last_da, succ, streak, best_ds
            },
            "nodes": picks,
            "k": chosen_k,
            "pool": pool_digest or {}
...
def run(initial_netlist: Any, max_iters: int=MAX_ITERS, *,
    ...
    # === gnnre only: refresh security once every 10 rounds ===

    def _gnnre_should_refresh(iter_idx: int) -> bool:
        return (iter_idx % GNNRE_UPDATE_EVERY) == 0

    # Record the most recent "real evaluation" security (used to hold it on non-refresh rounds)
    ...
    for t in range(max_iters):
        ...
        if eval_backend == "gnnre":
            if _gnnre_should_refresh(t):
                ...
                if last_full_security is not None:
                    ...
                    before = Scores(security=last_full_security, qor=before.qor)  # <- fix

        if best_scores is None or before.security < best_scores.security:
            best_netlist, best_scores = curr, before

        # ---------- Early stop (only valid on the corresponding backend) ----------
        # Note: gnnre only checks early stop on "refresh rounds", to avoid using the "held" security.
        if eval_backend == "omla" and before.security <= EARLY_STOP_FOR_OMLA:
            ...
            _log_iter(
                logger, phase="security", iter_idx=t, timing_s=(time.time()-start),
                eval_backend=eval_backend,
                netlist_before=str(curr),
                netlist_after=str(curr),
                before=before, after=after,
                ds_rel=ds_rel, da_rel=da_rel,
                picks=picks, chosen_combo_name=chosen_combo_name,
                chosen_use_gates=chosen_use_gates, chosen_k=chosen_k,
                pool_digest=pool_digest, rl_alloc=alloc, rl_probs=probs, rl_feats=feats,
                reward_node=reward_node, optim_notes=res.note,
                best_scores=best_scores, early_stop="omla_threshold"
            )
            return best_netlist
        if eval_backend == "gnn4ip" and before.security <= EARLY_STOP_FOR_GNN4IP:
            ...
            _log_iter(
                ...
                pool_digest=pool_digest, rl_alloc=alloc, rl_probs=probs, rl_feats=feats,
                reward_node=reward_node, optim_notes=res.note,
                best_scores=best_scores, early_stop="gnn4ip_threshold"
            )
            return best_netlist
        if eval_backend == "gnnre" and _gnnre_should_refresh(t) and before.security <= EARLY_STOP_FOR_GNNRE:
            ...
            _log_iter(
                ...
                pool_digest=pool_digest, rl_alloc=alloc, rl_probs=probs, rl_feats=feats,
                reward_node=reward_node, optim_notes=res.note,
                best_scores=best_scores, early_stop="gnnre_threshold"
            )
            return best_netlist
        ...
        # ---------- Select nodes (from the RL pool; step one) ----------
        ...
        # ---------- Select gate combo (step two; pass picks / pool_digest) ----------
        ...
        # ---------- Select k (step three; pass picks / pool_digest / combo) ----------
        ...
        if eval_backend == "gnnre":
            if _gnnre_should_refresh(t):
                ...
                # Non-refresh round: hold security, keeping it consistent with before
                if last_full_security is not None:
        ...
        # ---------- Early stop (after; gnnre only checks on refresh rounds) ----------
        ...
        if RL_ENABLE and rl is not None:
            from collections import Counter
            cnt = Counter(inst2bucket.get(x, "OTHER") for x in picks)
            chosen_alloc = [{"bucket": bk, "quota": q} for bk, q in cnt.items()]
            try:
                rl.update(chosen_alloc, probs, feats, TRY_PER_ROUND, reward_node)
                rl.save()
            except Exception as e:
        ...
        # On non-refresh rounds ds_rel=0 (because security is held), i.e. driven by area only
        ds_hist.append(ds_rel)
        da_hist.append(da_rel)
        ds_rel_prev = ds_rel
        da_rel_prev = da_rel
        # r_k = K_AREA_WEIGHT * da_rel - K_SEC_PENALTY_LAMBDA * max(0.0, -ds_rel)
        # r_k = max(-1.0, min(1.0, r_k))
        # if krl is not None and pi_k is not None:
        #     try:
        #         krl.update(chosen_k, pi_k, r_k)
        #         krl.save()
        #     except Exception as e:
        #         print("[RL-K] update failed:", e)

        # ---------- Slow-drop statistics + history ----------
        ...
        _log_iter(
            ...
            eval_backend=eval_backend,
            netlist_before=str(best_netlist if before.security == (best_scores.security if best_scores else before.security) else curr),  # rough record
            netlist_after=str(curr),
            before=before,
            after=after,
            ds_rel=ds_rel,
...
def run_area(initial_netlist: Any, max_iters: int=20, *,
             ...
             batch_size: int=64, eval_backend: str="omla", logger: Optional[ExperimentLogger]=None) -> Any:
    """
    Phase two: area optimization (uses LLM calls).
    Goal: reduce area without letting security worsen.
    """
    ...
    for t in range(max_iters):
        ...
        chosen_k = llm_choose_k_area_priority(
            ...
            combo_name=chosen_combo_name,
            use_gates=chosen_use_gates,
            prefer_small_k=True,   # bias toward small k to reduce complexity
            k_min=1, k_max=3,
            last_k=last_k_used,
            picked_nodes=picks, pool_digest=pool_digest
        )
        ...
        if (ds_rel < -SEC_TOL) or (da_rel <= AREA_TOL):
            ...
            _log_iter(
                ...
                phase="area",
                iter_idx=t,
                timing_s=0.0,  # if needed, add time.time()-start; put start=time.time() at the top of this function
                eval_backend=eval_backend,
                netlist_before=str(curr),
                netlist_after=str(curr),    # rejected, not updated
                before=before,
                after=after,
                ds_rel=ds_rel,
                da_rel=da_rel,
...
# ================== RL-ONLY OVERRIDES (drop-in; no LLM at all) ==================
# Placed at the end of the file, before __main__; these override the same-named llm_* decision functions

# Small helpers: scoring and per-group cap
def _rl_group_cap_ok(inst: str, cap: int, _grp_cnt: Dict[str, int]) -> bool:
    gid = opt_group_id(inst)
    return _grp_cnt.get(gid, 0) < cap

def _score_node_for_pick(netlist: str, inst: str) -> float:
    # Consistent with your existing heuristic: level + 2*fanout + 0.5*fanin
    f = extract_node_features(netlist, inst)
    return float(f.get("level", 0)) + 2.0 * float(f.get("fanout", 0)) + 0.5 * float(f.get("fanin", 0))

# ---------- 1) Override: node selection (RL/heuristic, no LLM) ----------
def llm_choose_from_pool(netlist: str, pool: List[str], try_per_round: int,
                         security: float, area: float, prefer_base: bool,
                         *, work_dir: Optional[str], circuit_name: Optional[str]) -> List[str]:
    if not pool:
        return []
    pool = list(dict.fromkeys(pool))  # dedupe, preserving order
    group_cnt: Dict[str, int] = {}
    picks: List[str] = []

    # First pick base per prefer_base, then fill with opt; both sorted by structural heuristic descending
    base = [x for x in pool if IS_BASE(x)]
    opt  = [x for x in pool if IS_OPT(x)]
    base.sort(key=lambda x: _score_node_for_pick(netlist, x), reverse=True)
    opt .sort(key=lambda x: _score_node_for_pick(netlist, x), reverse=True)

    def _take_from(cands: List[str], cap_per_group: int):
        nonlocal picks
        for nid in cands:
            if len(picks) >= try_per_round:
                break
            if _rl_group_cap_ok(nid, cap_per_group, group_cnt):
                picks.append(nid)
                gid = opt_group_id(nid)
                group_cnt[gid] = group_cnt.get(gid, 0) + 1

    if prefer_base:
        _take_from(base, MAX_PER_GROUP_IN_PICKS)
        if len(picks) < try_per_round:
            _take_from(opt, MAX_PER_GROUP_IN_PICKS)
    else:
        _take_from(opt, MAX_PER_GROUP_IN_PICKS)
        if len(picks) < try_per_round:
            _take_from(base, MAX_PER_GROUP_IN_PICKS)

    # Fallback: if still short, fill from all remaining candidates
    if len(picks) < try_per_round:
        rest = [x for x in pool if x not in picks]
        rest.sort(key=lambda x: _score_node_for_pick(netlist, x), reverse=True)
        for x in rest:
            if len(picks) >= try_per_round:
                break
            if _rl_group_cap_ok(x, MAX_PER_GROUP_IN_PICKS, group_cnt):
                picks.append(x)
                gid = opt_group_id(x)
                group_cnt[gid] = group_cnt.get(gid, 0) + 1
...
# ---------- 2) Override: combo selection (history + rules, no LLM) ----------
def llm_choose_combo_by_history(
    security: float,
    area: float,
    *,
    ...
    allow_set, block_set = _load_fixed_lists(work_dir)
    cand_names = list(GATE_COMBOS.keys())
    if allow_set:
        cand_names = [c for c in cand_names if c in allow_set]
    cand_names = [c for c in cand_names if c not in block_set]
    if not cand_names:
        cand_names = list(GATE_COMBOS.keys())

    # History-based scoring: ema_ds first, then ema_da; slight exploration noise may be added
    scored = []
    for name in cand_names:
        h = _COMBO_PERF.get(name, {})
        ema_ds = float(h.get("ema_ds", 0.0))
        ema_da = float(h.get("ema_da", 0.0))
        bad    = _bad_combo(name)
        score  = 2.0 * ema_ds + 0.8 * ema_da - (0.6 if bad else 0.0)
        # When security is very low, slightly prefer combos that "slightly reduce area"
        if security <= SEC_FOCUS_THRESHOLD and (ema_da > 0.0):
            score += (0.25 if 0.0 < ema_da <= SMALL_AREA_DROP else 0.05)
        # If last round area dropped but security clearly rose, down-weight more aggressive items
        if (last_da_rel is not None and last_ds_rel is not None and last_da_rel > 0 and last_ds_rel < -0.05):
            if ema_da < -AREA_TOL_UP:
                score -= 0.2
        scored.append((score, name))

    scored.sort(key=lambda x: x[0], reverse=True)
    choice = scored[0][1] if scored else "C14_INV_AND_OR_LOGIC"

    # When a style switch is needed, avoid reusing last round's combo
    if need_switch and last_combo and choice == last_combo:
        for _, nm in scored:
            if nm != last_combo:
                choice = nm
...
# area-phase version (do not worsen security; prioritize reducing area)
def llm_choose_combo_area_priority(
    security: float,
    area: float,
    *,
    ...
    table = GATE_COMBOS_AREA if GATE_COMBOS_AREA else GATE_COMBOS
    cand_names = list(table.keys())
    allow_set, block_set = _load_fixed_lists(work_dir)
    if allow_set:
        cand_names = [c for c in cand_names if c in allow_set]
    cand_names = [c for c in cand_names if c not in block_set]
    ...
    for name in cand_names:
        ...
        ema_ds = float(h.get("ema_ds", 0.0))
        ema_da = float(h.get("ema_da", 0.0))
        if ema_ds < -SEC_TOL:  # may worsen security -> down-weight
            penalty = 0.6
        else:
            penalty = 0.0
        # Area-first, while constraining to a "slight decrease" for more stability
        prefer = (0.35 if 0.0 < ema_da <= SMALL_AREA_DROP else (0.15 if ema_da > 0.0 else 0.0))
        score = 1.2 * ema_da + 0.5 * ema_ds + prefer - penalty
        scored.append((score, name))

    scored.sort(key=lambda x: x[0], reverse=True)
    choice = scored[0][1] if scored else cand_names[0]
    if need_switch and last_combo and choice == last_combo:
        for _, nm in scored:
            if nm != last_combo:
                choice = nm
...
# ---------- 3) Override: k selection (bandit, no LLM) ----------
# Standalone k-bandit (same policy as RLKPolicy in run(); self-contained here to avoid changing the main flow)
_K_BANDIT = RLKPolicy(path=RLK_POLICY_PATH, alpha=K_ALPHA, baseline_beta=K_BASELINE_BETA)

def _k_reward_from_hist(recent_ds_rel: Optional[List[float]], recent_da_rel: Optional[List[float]]) -> float:
    ds = float(_ema_list(list(recent_ds_rel or [])))
    da = float(_ema_list(list(recent_da_rel or [])))
    r  = K_AREA_WEIGHT * da - K_SEC_PENALTY_LAMBDA * max(0.0, -ds)
    # Clamp to [-1,1]
...
def llm_choose_k(
    *,
    ...
    # First select with the bandit
    global _K_BANDIT
    # Temporarily adjust the bandit's action dimension
    _K_BANDIT.h = _K_BANDIT.h[: (k_max - k_min + 1)] or [0.0] * (k_max - k_min + 1)
    k, pi = _K_BANDIT.select(temperature=K_TEMP)

    # Reward the bandit directly with the latest round's (ds_rel, da_rel) (keeping online learning)
    r_k = _k_reward_from_hist(recent_ds_rel, recent_da_rel)
    try:
        _K_BANDIT.update(k, pi, r_k)
        _K_BANDIT.save()
    except Exception:
        pass
    return int(max(k_min, min(k_max, k)))

# area-phase version: same bandit, but the reward emphasizes area gain while not worsening security
def llm_choose_k_area_priority(
    *,
    security: float,
    area: float,
    ...
    # Apply a harder penalty on security (no worsening)
    SEC_TOL  = float(os.getenv("SEC_NO_WORSE_TOL", "5e-4"))
    ds = float(_ema_list(list(recent_ds_rel or [])))
    da = float(_ema_list(list(recent_da_rel or [])))
    # Treat security worsening as a strong negative reward; otherwise a positive reward for area improvement
    penal = 1.0 if ds < -SEC_TOL else 0.0
    r_k = (K_AREA_WEIGHT * da) - (K_SEC_PENALTY_LAMBDA + penal) * max(0.0, -ds)
    r_k = max(-1.0, min(1.0, r_k))
...
#     parser = argparse.ArgumentParser(description="LLM-driven combo (history-only, fixed allow/block lists) + RL(node) + RL(k)")
...
#                         help="subcircuit_opt working directory (e.g. tmp) - the evaluator will use ../{work_dir} internally")
#     parser.add_argument("--circuit_name", type=str, required=True,
#                         help="dataset circuit name (e.g. c1355 / gnn4ip_c432_BE280); top will be set to locked_<name> (OMLA) or <name> (GNN4IP)")
#     parser.add_argument("--batch_size", type=int, default=int(os.environ.get("OMLA_BATCH","64")),
#                         help="OMLA attack batch size (ignored for GNN4IP)")
#     parser.add_argument("--eval_backend", type=str, choices=["omla","gnn4ip", "gnnre", "trojan"], default=os.environ.get("EVAL_BACKEND","omla"),
#                         help="choose the security evaluation backend: omla or gnn4ip")
...
#         "script": "llm_exo_RL_fresh_clean.py",
#         "args": {
...
#     # Update the parameters needed by subcircuit_opt
...
#     print("=== Stage 1: Security reduction ===")
#     final_netlist = run(
#         args.netlist,
#         max_iters=args.max_iters,
#         work_dir=args.work_dir,
...
#     # # === Stage 2: reduce area (ensuring security does not get worse) ===
#     # print("\n=== Stage 2: Area reduction ===")
#     # final_netlist_area = run_area(
#     #     final_netlist,
#     #     max_iters=args.max_iters,   # can be set separately, e.g. 20
#     #     work_dir=args.work_dir,
#     #     circuit_name=args.circuit_name,
#     #     batch_size=args.batch_size,
#     #     eval_backend=args.eval_backend,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="LLM-driven combo (history-only, fixed allow/block lists) + RL(node) + RL(k)"
    )
    parser.add_argument("--netlist", type=str, default="locked_c1355_0_1_0_0_0_1_flat.v")
    parser.add_argument("--work_dir", type=str, default=os.environ.get("WORK_DIR","tmp"),
                        help="subcircuit_opt working directory (e.g. tmp) - the evaluator will use ../{work_dir} internally")
    ...
    parser.add_argument("--batch_size", type=int, default=int(os.environ.get("OMLA_BATCH","64")),
                        help="OMLA attack batch size (ignored for GNN4IP)")
    parser.add_argument("--eval_backend", type=str, choices=["omla","gnn4ip", "gnnre", "trojan"],
                        default=os.environ.get("EVAL_BACKEND","omla"),
                        help="choose the security evaluation backend: omla / gnn4ip / gnnre / trojan")
    parser.add_argument("--max_iters", type=int, default=MAX_ITERS, help="maximum number of iterations")

    # ---- New: variability control for multiple experiment groups ----
    parser.add_argument("--seed", type=int, default=None,
                        help="random seed; if not given, a time-based seed is used")
    parser.add_argument("--reset_state", action="store_true",
                        help="reset RL / visit records / historical performance before start (rl_*_policy.json, visited_opt.json, combo_perf.json)")
    parser.add_argument("--policy_tag", type=str, default=None,
                        help="use a separate policy-file suffix for this experiment, e.g. --policy_tag g1")

    # ---- New: two independent ablation modes ----
    parser.add_argument("--ablation", choices=["none", "no_mapping", "fixed_k"], default="none",
                        help="ablation choice: no_mapping=no gate mapping; fixed_k=fixed k; none=normal flow")
    parser.add_argument("--fixed_k_val", type=int, default=5,
                        help="k value used in fixed_k mode")
    parser.add_argument("--area_phase", action="store_true",
                        help="if specified, run the corresponding area-phase flow")

    args = parser.parse_args()

    # ============== Experiment variability: seed / separate policy files / state reset ==============
    # 1) Random seed
    ...
    # 2) Separate policy files (avoid cross-contamination between groups)
    if args.policy_tag:
        RL_POLICY_PATH  = f"rl_bucket_policy_{args.policy_tag}.json"
        RLK_POLICY_PATH = f"rl_k_policy_{args.policy_tag}.json"

    # 3) Optional: reset state files
    if args.reset_state:
        import os, glob
        for p in [
            RL_POLICY_PATH,
            RLK_POLICY_PATH,
            "visited_opt.json",
            os.path.join(args.work_dir or "", "combo_perf.json"),
        ]:
            try:
                if os.path.isfile(p):
                    os.remove(p)
    ...
    LOGGER.log("run_start", {
        ...
        "args": {
            ...
            "fixed_k_val": args.fixed_k_val if args.ablation == "fixed_k" else None,
            "area_phase": bool(args.area_phase),
    ...
    # ============== Main-flow routing (mutually exclusive, pick one of three) ==============
    out_security = None
    out_area = None

    if args.ablation == "no_mapping":
        # Experiment A: no gate mapping
        if args.area_phase:
            print("\n=== Ablation A (Area): WITHOUT gate mapping ===")
            out_area = run_area_ablate_nomap(
                args.netlist, max_iters=args.max_iters,
                work_dir=args.work_dir, circuit_name=args.circuit_name,
                batch_size=args.batch_size, eval_backend=args.eval_backend, logger=LOGGER
            )
            print("[Ablation area no-mapping DONE]:", out_area)
            LOGGER.log("run_end", {"final_netlist_area": str(out_area), "ablation": "no_mapping"})
        else:
            print("\n=== Ablation A (Security): WITHOUT gate mapping ===")
            out_security = run_ablate_nomap(
                args.netlist, max_iters=args.max_iters,
                work_dir=args.work_dir, circuit_name=args.circuit_name,
                batch_size=args.batch_size, eval_backend=args.eval_backend, logger=LOGGER
            )
            print("[Ablation no-mapping DONE]:", out_security)
            LOGGER.log("run_end", {"final_netlist_security": str(out_security), "ablation": "no_mapping"})

    elif args.ablation == "fixed_k":
        # Experiment B: fixed k
        k_fixed = int(max(K_MIN, min(K_MAX, int(args.fixed_k_val))))
        if args.area_phase:
            print(f"\n=== Ablation B (Area): FIXED k={k_fixed} ===")
            out_area = run_area_ablate_fixed_k(
                args.netlist, k_fixed=k_fixed, max_iters=args.max_iters,
                work_dir=args.work_dir, circuit_name=args.circuit_name,
                batch_size=args.batch_size, eval_backend=args.eval_backend, logger=LOGGER
            )
            print("[Ablation area fixed-k DONE]:", out_area)
            LOGGER.log("run_end", {"final_netlist_area": str(out_area), "ablation": "fixed_k", "k_fixed": k_fixed})
        else:
            print(f"\n=== Ablation B (Security): FIXED k={k_fixed} ===")
            out_security = run_ablate_fixed_k(
                args.netlist, k_fixed=k_fixed, max_iters=args.max_iters,
                work_dir=args.work_dir, circuit_name=args.circuit_name,
                batch_size=args.batch_size, eval_backend=args.eval_backend, logger=LOGGER
            )
            print("[Ablation fixed-k DONE]:", out_security)
            LOGGER.log("run_end", {"final_netlist_security": str(out_security), "ablation": "fixed_k", "k_fixed": k_fixed})

    else:
        # Normal LLM+RL main flow
        if args.area_phase:
            print("\n=== Normal Area Phase ===")
            out_area = run_area(
                args.netlist,
            ...
            print("[Area DONE]:", out_area)
            LOGGER.log("run_end", {"final_netlist_area": str(out_area), "ablation": "none"})
        else:
            print("\n=== Normal Security Phase ===")
            out_security = run(
                args.netlist,
                max_iters=args.max_iters,
                work_dir=args.work_dir,
            ...
            print("[Security DONE]:", out_security)
            LOGGER.log("run_end", {"final_netlist_security": str(out_security), "ablation": "none"})
